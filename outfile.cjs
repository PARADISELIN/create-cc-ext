    ...
    function patch(fs4) {
      ...
      function patchLchmod(fs5) {
        fs5.lchmod = function(path3, mode, callback) {
          fs5.open(path3, constants.O_WRONLY | constants.O_SYMLINK, mode, function(err, fd) {
            if (err) {
              if (callback)
                callback(err);
              return;
  ...
  "node_modules/graceful-fs/graceful-fs.js"(exports, module2) {
    ...
    function patch(fs5) {
      ...
      function createReadStream(path3, options2) {
        return new fs5.ReadStream(path3, options2);
      }
      function createWriteStream(path3, options2) {
        return new fs5.WriteStream(path3, options2);
      }
...
var require_node_figlet = __commonJS({
  "node_modules/figlet/lib/node-figlet.js"(exports, module2) {
    ...
    figlet2.loadFont = function(name, next) {
      ...
      fs4.readFile(path3.join(fontDir, name + ".flf"), { encoding: "utf-8" }, function(err, fontData) {
        if (err) {
          return next(err);
        }
        fontData = fontData + "";
    ...
    figlet2.loadFontSync = function(name) {
      ...
      var fontData = fs4.readFileSync(path3.join(fontDir, name + ".flf"), { encoding: "utf-8" });
      fontData = fontData + "";
      return figlet2.parseFont(name, fontData);
    };
    figlet2.fonts = function(next) {
      var fontList = [];
      fs4.readdir(fontDir, function(err, files) {
        if (err) {
          return next(err);
        }
    ...
    figlet2.fontsSync = function() {
      var fontList = [];
      fs4.readdirSync(fontDir).forEach(function(file) {
        if (/\.flf$/.test(file)) {
          fontList.push(file.replace(/\.flf$/, ""));
        }
      });
...
var GLOBAL_CREATOR_DIRNAME = ".CocosCreator";
var GLOBAL_COCOS_DIRNAME = ".Cocos";
var EXTENSION_DIRNAME = "extensions";

// lib/utils/cocos.ts
function getGlobalCocosPath() {
  return import_path.default.join(import_os.default.homedir(), GLOBAL_COCOS_DIRNAME);
}
function getGlobalCreatorExtensionPath() {
  return import_path.default.join(import_os.default.homedir(), GLOBAL_CREATOR_DIRNAME, EXTENSION_DIRNAME);
}
function getCocosEditorsInfo() {
  const cocosPath = getGlobalCocosPath();
  const editorProfilePath = import_path.default.join(cocosPath, "profiles", "editor.json");
  if (!import_fs_extra2.default.pathExistsSync(editorProfilePath)) {
    return null;
  }
  const content = import_fs_extra2.default.readFileSync(editorProfilePath, { encoding: "utf-8" });
...
function getCocosProjectsInfo() {
  const cocosPath = getGlobalCocosPath();
  const projectProfilePath = import_path.default.join(cocosPath, "profiles", "project.json");
  if (!import_fs_extra2.default.pathExistsSync(projectProfilePath)) {
    return null;
  }
  const content = import_fs_extra2.default.readFileSync(projectProfilePath, { encoding: "utf-8" });
  const profile = JSON.parse(content);
  if (!profile) {
    return null;
  }
...
function getExtensionTargetPath(extensionName, project) {
  if (project != null) {
    ...
    return import_path.default.join(projectPath, EXTENSION_DIRNAME, extensionName);
  }
  ...
  },
  {
    ...
    type: (prev, values) => {
      if (values.extensionScope === 1 && !editorsInfo) {
        throw new Error(red("\u2716") + " Operation cancelled, you may not have any versions of the editor installed.");
      }
...
function init() {
  return __async(this, null, function* () {
    ...
    if (shouldOverwrite) {
      ...
      import_fs_extra3.default.mkdirSync(root);
    }
    console.log(`
...
`);
    ...
    import_fs_extra3.default.writeFileSync(import_path2.default.resolve(root, "README.md"), readmeEN);
    import_fs_extra3.default.writeFileSync(import_path2.default.resolve(root, "README-CN.md"), readmeCN);
